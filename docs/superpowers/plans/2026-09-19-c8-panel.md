...
Nach Task 11 hat ein Prüfer ohne Vorwissen den Stand geprüft, am 2026-09-20. Er bekam nur den
Worktree, die Spec und die Testbefehle. Zuerst wurde die Spec nachgezogen (Brain `549d8d3`), dann
der Code, mit je einem Test und grüner Suite (`da11bb2`). Die Codeblöcke der Tasks 1 bis 10 zeigen
den Stand davor.

| # | Befund | Behoben durch |
|---|---|---|
| 1 | Ein gescheiterter Lesebefehl zeigte nur einen leeren Zustand: „Noch keine Prognose.“, „Noch kein Plan“, eine leere Liste. Spec 8 verlangt die Meldung mit dem Text des Fehlers | Jeder Leser gibt seinen Fehler zurück, `_fehlerMelden` zeigt den ersten je Lauf; Spec 8 nennt das Lesen ausdrücklich |
| 2 | Ein Fehler des Dienstes mit dem Schlüssel einer Warnung (`fahrer_doppelt`) suchte im Formular `.fehler[data-ort="fahrer"]`. Das Feld gibt es nicht: `zeige` hätte geworfen, das Speichern wäre stumm stehen geblieben | `FEHLERORTE` und `fehlerOrt` in `pruefung.js`; alles andere geht als Meldung unten. Spec 7.3 nennt die vier Orte |
| 3 | `zeige` schrieb rohen Text in `innerHTML`. Die Platzhalter eines Dienstfehlers tragen Namen aus Home Assistant | `zeige` escapet über `html` |
| – | Testlücke: die Hinweise je Termin (`overlap`, `driver_busy`) rendert niemand im Test | Test in `ansichten.test.mjs`, mit Titel, ohne Titel und escaped |
| – | Testlücke: die Zusammenhangsprüfung des günstigsten Zeitraums ließ sich entfernen, ohne dass ein Test rot wurde | Test in `preise.test.mjs` mit einer Lücke zwischen den Slots |

Jede Behebung ist gegen eine Verfälschung geprüft, alle drei Gegenproben rot. Die Vorschau bekam
den Schalter „Lesefehler“, damit die Meldung sichtbar ist. Suite danach: 460 pytest, 73 Node-Tests.
